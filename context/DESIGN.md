...
- **Do** — при добавлении нового цвета/варианта сразу добавить его в соответствующий реестр (`SURFACE_VARIANTS`, `semanticRoles`/`chartKeys` и т.п. в `foundation/`), а не только использовать в JSX где-то — иначе он невидим для всего, что читает реестр программно (см. `context/AGENTS.md`).
