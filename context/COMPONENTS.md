...
- Разделитель — между КРУПНЫМИ смысловыми блоками, не перед каждым элементом. Три блока, не пять: (шапка + статус-тег вместе, это одно «сейчас с этим человеком») → (метрики) → (CTA). Тег после шапки — без своего отдельного `<Separator />`, это тот же блок, ему нужен только `gap`. Живая ошибка, которую поймали: агент поставил разделитель перед каждым элементом (шапка / тег / метрики / кнопка — 3 линии в одной небольшой карточке) — визуально расшумело карточку, а не структурировало.
