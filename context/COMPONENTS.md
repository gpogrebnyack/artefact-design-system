...
- ⚠️ **демо, не компонент** — существует только как код внутри Storybook-
  истории, ничего не экспортируется для импорта. Если нужно использовать —
...
`Avatar`, `Badge`, `Button`, `Checkbox`, `Input`, `RadioGroup`, `Select`, `Switch`, `Toggle`/`ToggleGroup`, `Tooltip`, `NavigationMenu`, `Progress`, `Separator` (в Storybook: `Divider`), `Dialog` (в Storybook: `Modal`).
...
**⚠️ `Primitives/Chart` — не компонент.** Story `Chart.stories.tsx` рисует `Sparkline`/`ScoreRing` прямо в `render()` — это proof-of-concept, что Recharts+`ChartContainer` покрывает то, что раньше было кастомным SVG. Ничего не экспортируется — если понадобится настоящий спарклайн-компонент, его нужно сначала выделить из этой истории в реальный файл.
...
Это тоже стандартные shadcn-компоненты, но они — композиционные оболочки (слоты Header/Content/Footer, список пунктов и т.п.), а не атомарный контрол, поэтому в Storybook они лежат в `Components/*`, не в `Primitives/*`. API — см. shadcn-документацию.

**⚠️ `Components/Chart/MetricCard` — не компонент.** Как и `Primitives/Chart`, это `render()`-демо в сторис-файле (`Card` + `Text` + `LineChart`, собранные вручную) — не экспортируется, есть только как образец композиции, скопируй руками, если нужен такой паттерн.
...
Секции — намеренно немногочисленные (стоящее правило: `Sections` остаётся компактным, а не превращается во вторую копию `Components`; страницные блоки без повторяющегося прецедента на нескольких реальных экранах продукта собираются прямо в сторис страницы, не пакуются в новую секцию раньше времени).
...
## Демо/сторис в этом репозитории

`src/stories/*` — витрина для разработки самой системы (см. `npm run storybook` в `context/AGENTS.md`), не публикуется. Здесь нет и не должно быть сборки целой страницы продукта («Pages») — это дело потребляющего приложения; в этом репозитории демонстрируются только сами Foundation/Primitives/Components/Sections по отдельности.
