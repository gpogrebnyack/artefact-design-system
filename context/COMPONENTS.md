...
Этот репозиторий — сам пакет (`src/` → `dist/`), не витрина. Storybook и `*.stories.tsx` живут в потребляющем приложении (например, `shadcn-experiment`), не здесь — см. `context/AGENTS.md` про то, как это распределено между репозиториями.
