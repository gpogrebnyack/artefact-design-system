...
const VARIANTS = {
  ...
  paper: {
    // Вложенная поверхность. Заливка — cream-alpha (--secondary), которая
    // композитится на ступень ТЕМНЕЕ родителя в любом контексте: карточка на
    // панели, чип на канве, карточка в карточке — всегда отделяется сама, без
    // hairline. Раньше был off-scale #fff и сливался с cream-1-панелью.
    style: { background: color.secondary } as CSSProperties,
    description: "вложенная поверхность (на ступень темнее родителя) — карточки, чипы, поля",
  },
  outline: {
