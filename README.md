...
Шрифт (Struve, три начертания) — отдельные `.woff2` в `dist/fonts/`, на которые `style.css` ссылается относительным `url()` (паттерн Fontsource). Через бандлер (Vite/Next/webpack) это работает само — ассеты подхватываются и копируются. Если раздаёшь `dist/style.css` без бандлера — держи `dist/fonts/` рядом с ним.
